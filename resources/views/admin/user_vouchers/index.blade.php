                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-sm-auto">
                            <div class="mb-4">
                                <a href="{{ route('admin.user_vouchers.create') }}" class="btn btn-primary me-2">+ Thêm mới</a>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody>
                                @foreach ($User_vouchers as $User_voucher)
                                    <tr>
                                        ...
                                        <td>{{ $User_voucher->User ? $User_voucher->User->name : 'Không có' }}</td>
                                        <td>{{ $User_voucher->Voucher ? $User_voucher->Voucher->code : 'Không có mã' }}</td>
                                        <td>{{ $User_voucher->Voucher ? $User_voucher->Voucher->title :'không có tên' }}</td>
                                        {{-- <td>{{ optional($User_voucher->user)->name }}</td>
                                        <td>{{ optional($User_voucher->voucher)->code }}</td>
                                        <td>{{ optional($User_voucher->voucher)->title }}</td> --}}
                                        <td>{{ $User_voucher->usage_count }}</td>
                                        <td class="text-center">
                                            <a href="{{ route('admin.user_vouchers.edit', $User_voucher) }}" class="btn btn-warning btn-sm">
                                                <i class="fas fa-edit"></i> 
                                            </a>
                                            {{-- <form action="{{ route('admin.user_vouchers.destroy', $User_voucher) }}" method="POST"
